...
def main():
    ...
    # Plot 1: Overall average accuracy vs. beta (LINEAR)
    ...
    plot1_path = output_dir / "beta_sweep_avg_linear.png"
    plt.savefig(plot1_path, dpi=300, bbox_inches='tight')
    plt.close()
    logging.info(f"Saved linear average plot to {plot1_path}")

    # Plot 2: Per-benchmark accuracy vs. beta (LINEAR)
    ...
    plot2_path = output_dir / "beta_sweep_per_benchmark_linear.png"
    plt.savefig(plot2_path, dpi=300, bbox_inches='tight')
    plt.close()
    logging.info(f"Saved linear per-benchmark plots to {plot2_path}")
